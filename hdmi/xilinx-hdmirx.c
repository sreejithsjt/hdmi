...
static void RxHdcpAuthenticatedCallback(void *CallbackRef)
{
	...
	case XV_HDMIRXSS_HDCP_22:
		dev_dbg(xhdmi->dev,"HDCP 2.2 RX authenticated.\n");
		break;
	case XV_HDMIRXSS_HDCP_14:
		dev_dbg(xhdmi->dev,"HDCP 1.4 RX authenticated.\n");
		break;
	}
}
...
static void RxHdcpEncryptionUpdateCallback(void *CallbackRef)
{
	...
	dev_dbg(xhdmi->dev,"HDCP RX encryption changed; now %s.\n",
...
static int xhdmi_probe(struct platform_device *pdev)
{
	...
	XV_HdmiRx_ConfigTable[instance].AxiLiteClkFreq = axi_clk_rate;
	...
	XV_HdmiRxSs_SetCallback(HdmiRxSsPtr, XV_HDMIRXSS_HANDLER_BRDGOVERFLOW,
		RxBrdgOverflowCallback, (void *)xhdmi);
